...
server.registerTool(
    'jules_show_source',
    {
        ...
        inputSchema: z.object({
            name: z.string()
                .max(256)
                .regex(/^sources\/[^/]+\/[^/]+\/[^/]+$/, { message: "Source name must match format 'sources/github/owner/repo'" })
...
server.registerTool(
    'jules_show_session',
    {
        ...
        inputSchema: z.object({
            session_id: z.string()
                .max(256)
                .regex(/^(sessions\/)?\d+$/, { message: "Session ID must be numeric or match format 'sessions/numeric'" })
    ...
    'jules_show_activity',
    {
        ...
        inputSchema: z.object({
            activity_name: z.string()
                .max(256)
                .regex(/^sessions\/\d+\/activities\/\d+$/, { message: "Activity name must match format 'sessions/ID/activities/ID'" })
