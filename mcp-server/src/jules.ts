...
server.registerTool(
    'jules_show_source',
    {
        ...
        inputSchema: z.object({
            name: z.string()
                .max(256)
                .regex(/^sources\/[^/]+\/[^/]+\/[^/]+$/, { message: "Invalid source format (e.g., sources/github/owner/repo)" })
...
server.registerTool(
    'jules_show_session',
    {
        ...
        inputSchema: z.object({
            session_id: z.string()
                .max(256)
                .regex(/^(sessions\/)?\d+$/, { message: "Invalid session ID format" })
    ...
    'jules_show_activity',
    {
        ...
        inputSchema: z.object({
            activity_name: z.string()
                .max(256)
                .regex(/^sessions\/\d+\/activities\/\d+$/, { message: "Invalid activity name format" })
