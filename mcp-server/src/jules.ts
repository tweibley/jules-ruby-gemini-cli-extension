...
// Input limits to match backend constraints and prevent errors
const MAX_PROMPT_LENGTH = 50000;
const MAX_TITLE_LENGTH = 1000;
const MAX_ID_LENGTH = 256;
const MAX_GENERIC_LENGTH = 1024;
    ...
    'jules_show_session',
    {
        ...
        inputSchema: z.object({
            session_id: z.string()
                ...
                .refine(val => /^(sessions\/)?\d+$/.test(val), { message: "Session ID must be numeric (e.g., 12345678) or use the full path (sessions/12345678)" })
    ...
    'jules_create_session',
    {
        ...
        inputSchema: z.object({
            ...
            prompt: z.string()
                ...
                .describe('Task description for Jules to work on (max 50k chars). More detail leads to better results.'),
            title: z.string().optional()
                .refine(val => !val || !val.startsWith('-'), { message: "Title cannot start with '-'" })
                .refine(val => !val || val.length <= MAX_TITLE_LENGTH, { message: `Title must be under ${MAX_TITLE_LENGTH} characters` })
                .describe('Optional title for the session (short and descriptive)'),
