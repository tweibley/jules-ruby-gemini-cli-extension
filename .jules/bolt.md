...
## 2025-12-22 - [JSON Parsing and String Trimming]
**Learning:** In V8, `String.prototype.trim()` on large strings (e.g. 10MB) requires a full copy/allocation, which can be expensive (100ms+). Since `JSON.parse` ignores whitespace, trimming the string before parsing is redundant work that degrades performance.
**Action:** When capturing output destined for `JSON.parse`, skip `trim()` to save memory and CPU cycles.
